...
if [[ $# -lt 2 ]]; then
   ...
   case $1 in
           ...
           "-c") riscv32-unknown-elf-gcc -Wl,-Ttext=0x0000 -Os -fPIC -o riscv_raw_elf $2 -lm
   esac

   elf2hex 1 32768 riscv_raw_elf > riscv_hex 

   sed -n '1~4p' riscv_hex >> phelmino_ram_A.txt 
   sed -n '2~4p' riscv_hex >> phelmino_ram_B.txt 
   sed -n '3~4p' riscv_hex >> phelmino_ram_C.txt 
   sed -n '4~4p' riscv_hex >> phelmino_ram_D.txt 
fi;
